      ...
      {businesses.length === 0 ? (
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {businesses.map((b) => (
            ...
            >
              <span style={{ fontSize: 11, color: "#D86F52", fontWeight: 600 }}>
                {b.category}
              </span>
              <h3 style={{ fontWeight: 700, fontSize: 16, marginTop: 4 }}>{b.name}</h3>
              <p style={{ fontSize: 13, color: "#5C6B64", marginTop: 6, minHeight: 40 }}>
                {b.tagline}
              </p>

              <div
                className="w-full h-1.5 rounded-full overflow-hidden"
                style={{ backgroundColor: "#EEE9DF", marginTop: 12 }}
              >
                <div
                  className="h-full rounded-full"
                  style={{ width: `${b.percentFunded}%`, backgroundColor: "#D86F52" }}
                />
              </div>
              <div
                className="flex justify-between"
                style={{ fontSize: 12, color: "#5C6B64", marginTop: 8 }}
              >
                <span>£{b.raised.toLocaleString()} raised</span>
                <span>{b.percentFunded}% of £{b.fundingTarget.toLocaleString()}</span>
              </div>
              <div style={{ fontSize: 11, color: "#5C6B64", marginTop: 6 }}>
